/* Note: not solved by myself, using dp to reduce complexity */
#include <iostream>
...
public:
    bool wordBreak(string s, vector<string> &wordDict)
    {
        ...
        return wordBreakHelper(s, 0, dp, Words);
    }

private:
    bool wordBreakHelper(string &s, int start, vector<int> &dp, unordered_set<string> &Words)
    {
        ...
        for (int i = start; i < s.size(); i++)
        {
            ...
            {
                if (wordBreakHelper(s, i + 1, dp, Words))
                {
                    dp[start] = true;
                    return true;
                }
